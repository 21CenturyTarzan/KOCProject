...
//      .isWhitelisted(blockchain.account).call().then(result => result);
      ...
      let whiteList = 0;
      for (let i = 0; i < WL.whitelist.length; i ++) {
        if (WL.whitelist[i] === blockchain.account) {
          whiteList = 1;
          break;
        }
      }
...
//      .onlyWhitelisted().call().then(result => result);
      ...
      let onlyWhitelisted = 0;
      for (let i = 0; i < WL.onlywhitelist.length; i ++) {
        if (WL.onlywhitelist[i] === blockchain.account) {
          onlyWhitelisted = 1;
          break;
        }
      }
